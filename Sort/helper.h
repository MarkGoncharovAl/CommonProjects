...
namespace mpi
{
    ...
    namespace details {
        ...
        template <>
        MPI_Datatype getType<char>() { 
            return MPI_CHAR;
        }
        template <>
        MPI_Datatype getType<int>() { 
            return MPI_INT;
        }
        template <>
        MPI_Datatype getType<double>() { 
            return MPI_DOUBLE;
        }
        template <>
        MPI_Datatype getType<size_t>() { 
            return MPI_UINT64_T;
        }
    ...
    template <typename T>
    void send(T data, int rank, size_t size, int tag = 0)
    {
        if constexpr (std::is_pointer_v<T>) 
            act(MPI_Send(data, size, details::getType<std::remove_pointer_t<T>>(), rank, tag, MPI_COMM_WORLD));
        else
            act(MPI_Send(&data, size, details::getType<T>(), rank, tag, MPI_COMM_WORLD));
    }
    template <typename T>
    void send(std::vector<T> &data, int rank, size_t size = 0, int tag = 0)
    {
        if (size == 0)
            size = data.size();
        std::cout << size;
        act(MPI_Send(data.data(), size, details::getType<T>(), rank, tag, MPI_COMM_WORLD));
    }
    template <typename T> 
    void recv(T data, int rank, size_t size = 1, int tag = 0) requires (std::is_pointer_v<T>)
    {
        act(MPI_Recv(data, size, details::getType<std::remove_pointer_t<T>>(), rank, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE));
    }
    template <typename T>
    T recv(int rank, size_t size = 1, int tag = 0)
    {
        T data;
        act(MPI_Recv(&data, size, details::getType<T>(), rank, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE));
        return data;
    }
    template <typename T>
    std::vector<T> recvVector(int rank, size_t size, int tag = 0)
    {
        ...
        act(MPI_Recv(data.data(), data.size(), details::getType<T>(), rank, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE));
        return data;
    }
}