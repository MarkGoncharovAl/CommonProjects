...
scale_t processStart(int argc, char *argv[]) {
  ...
  desc.add_options()("help,h", "Information about options")(
      "time", "Time checker parallel")("size,s", po::value<size_t>(),
                                       "Size of result matrix")(
      "tau,t", po::value<double>(), "Time step")("x,x", po::value<double>(),
                                                 "Coordinate step");
...
void dumpShort(const std::vector<std::vector<double>> &data, int sizeX,
               int sizeT) {
...
void fillDataShort(std::function<double(double, double)> f,
                   std::vector<std::vector<double>> &data, scale_t scale,
                   int sizeX, int curT, int offset) {
...
void fillData(std::function<double(double, double)> f,
              std::vector<std::vector<double>> &data, scale_t scale, int sizeX,
              int offset) {
...
solveLinear(scale_t scale, std::function<double(double)> phi,
  ...
  std::for_each(data.begin(), data.end(),
                [scale](std::vector<double> &arr) { arr.resize(scale.size); });
...
std::vector<std::vector<double>>
solveParallel(scale_t scale, std::function<double(double)> phi,
              std::function<double(double)> psi,
              std::function<double(double, double)> f, int size, int rank) {
  std::vector<std::vector<double>> data(scale.size);
  ...
  if (rank == mpi::rank_main) {
    ...
    std::for_each(
        data.begin(), data.end(),
        [n = 0, &psi, scale, size_main](std::vector<double> &arr) mutable {
          arr.resize(size_main, 0);
          arr[0] = psi(scale.tau * n++);
        });
    std::generate(
        data[0].begin() + 1, data[0].end(),
        [n = 1, &phi, scale]() mutable { return phi(scale.h * n++); });

    mainThreadWork(scale, data, f, common_size, size);
  } else {
    double offset = size_main + common_size * (rank - 1) - 1;
    std::for_each(data.begin(), data.end(),
                  [common_size](std::vector<double> &arr) {
                    arr.resize(common_size + 1, 0);
                  });
    std::generate(data[0].begin(), data[0].end(),
                  [n = 0, &phi, scale, offset]() mutable {
...
void mainThreadWork(scale_t scale, std::vector<std::vector<double>> data,
                    std::function<double(double, double)> f, int common_size,
                    int size) {
  auto start_parallel_time = MPI_Wtime();
  double tauh = (scale.h + scale.tau) / (2 * scale.h * scale.tau);
  for (int t = 0; t < data.size() - 1; ++t) {
    fillDataShort(f, data, scale, data[0].size(), t);
    act(MPI_Send(&data[t + 1].back(), 1, MPI_DOUBLE, /*rank*/ 1, 0,
                 MPI_COMM_WORLD));
  }
  ...
  for (int i = 0; i < size - 1; ++i) {
    ...
    std::for_each(data_rank[i].begin(), data_rank[i].end(),
                  [common_size, i](auto &vec) {
                    vec.resize(common_size);
                    act(MPI_Recv(vec.data(), common_size, MPI_DOUBLE, i + 1, 1,
...
void commonThreadWork(scale_t scale, std::vector<std::vector<double>> data,
                      std::function<double(double, double)> f, int offset,
                      int rank) {
  double tauh = (scale.h + scale.tau) / (2 * scale.h * scale.tau);
  for (int t = 0; t < data.size() - 1; ++t) {
    act(MPI_Recv(data[t + 1].data(), 1, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE));
    fillDataShort(f, data, scale, data[0].size(), t, offset);
    act(MPI_Send(&data[t + 1].back(), 1, MPI_DOUBLE, /*rank*/ rank + 1, 0,
                 MPI_COMM_WORLD));
  }
  std::for_each(data.begin(), data.end(), [](auto &vec) {
    act(MPI_Send(vec.data() + 1, vec.size() - 1, MPI_DOUBLE, 0, 1,
                 MPI_COMM_WORLD));
  });
}
void lastThreadWork(scale_t scale, std::vector<std::vector<double>> data,
                    std::function<double(double, double)> f, int offset,
                    int rank) {
  double tauh = (scale.h + scale.tau) / (2 * scale.h * scale.tau);
  for (int t = 0; t < data.size() - 1; ++t) {
