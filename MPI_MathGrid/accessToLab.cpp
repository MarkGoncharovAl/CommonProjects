...
int change_num(int num) { return num + 1; }

constexpr int num_circles = 10000;
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < num_circles; ++i) {
    if (rank != mpi::rank_main) {
      act(MPI_Recv(&num, /*size*/ 1, MPI_INT, rank - 1, 0, MPI_COMM_WORLD,
                   MPI_STATUS_IGNORE));
    }
    if (rank != size - 1)
    ...
    if (rank == mpi::rank_main) {
      act(MPI_Recv(&num, /*size*/ 1, MPI_INT, /*rank*/ size - 1, 0,
