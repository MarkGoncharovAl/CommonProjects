...
void clM::BSort(std::vector<int> &data, const cl::Device &device,
                Sort sort /*= Sort::Incr*/) {
  // при создании Data_t
  // уже готовятся основные переменные
...
void clM::Data_t::BSort(std::vector<int> &data, Sort sort) {
  ...
  queue_.enqueueWriteBuffer(buffer, CL_TRUE, 0, new_size * sizeof(int),
                            data.data());
  ...
  cl::NDRange local_size =
      std::min(new_size / 2, device_.getInfo<CL_DEVICE_MAX_WORK_GROUP_SIZE>());
  ...
  std::cout << MLib::Color::Purple << "Time passed using GPU2: " << time_found
            << " mc" << std::endl
            << MLib::Color::Reset;

  //копируем обратно в исходный массив
  auto map_data = (int *)queue_.enqueueMapBuffer(buffer, CL_TRUE, CL_MAP_READ,
                                                 0, new_size * sizeof(int));
...
clMFunc::Devices_t clMFunc::GetDevices() {
  // Checking every platform
  std::vector<cl::Platform> platforms;
  cl::Platform::get(&platforms);
  if (platforms.size() == 0)
...
cl::Device clMFunc::AnalyseDevices(const Devices_t &devices, int argc,
                                   char *argv[]) {
  try {
    po::options_description desc("Allowed options");
    desc.add_options()("help", "information about devices")(
