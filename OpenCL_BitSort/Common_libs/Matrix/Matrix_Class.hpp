...
template <class Type_t> class Matrix_t final : private Buffer<Type_t> {
public:
  //  !Constructors
  //////////////////////////////////////////////////
  Matrix_t();
  Matrix_t(size_t init_row, size_t init_co,
           const Type_t &base_value = Type_t{});

  template <class Another_Type> Matrix_t(const Matrix_t<Another_Type> &that);
  ...
  public:
    ...
    MC_iterator<Type_t> cbegin() const
        noexcept(noexcept(MC_iterator<Type_t>{nullptr})) {
      return MC_iterator<Type_t>{data_};
    }
    MC_iterator<Type_t> cend() const
        noexcept(noexcept(MC_iterator<Type_t>{nullptr})) {
      return MC_iterator<Type_t>{data_ + size_};
    }
    M_iterator<Type_t> begin() noexcept(noexcept(M_iterator<Type_t>{nullptr})) {
      return M_iterator<Type_t>{data_};
    }
    M_iterator<Type_t> end() noexcept(noexcept(MC_iterator<Type_t>{nullptr})) {
      return M_iterator<Type_t>{data_ + size_};
    }
  ...
  MC_iterator<Type_t> cend() const
      noexcept(noexcept(MC_iterator<Type_t>{nullptr})) {
    return MC_iterator<Type_t>{data_ + size_.row * size_.col};
  }

  M_iterator<Type_t> begin() noexcept(noexcept(M_iterator<Type_t>{nullptr})) {
    return M_iterator<Type_t>{data_};
  }
  M_iterator<Type_t> end() noexcept(noexcept(M_iterator<Type_t>{nullptr})) {
    return M_iterator<Type_t>{data_ + size_.row * size_.col};
  }

  MS_iterator<Type_t>
  sbegin() noexcept(noexcept(MS_iterator<Type_t>{nullptr, nullptr})) {
    return MS_iterator<Type_t>{data_, data_ + size_.row * size_.col};
  }

  MS_iterator<Type_t>
  send() noexcept(noexcept(MS_iterator<Type_t>{nullptr, nullptr})) {
...
Matrix_t<Type_t>::Matrix_t() : Buffer<Type_t>(), size_({0, 0}) {}

template <class Type_t>
Matrix_t<Type_t>::Matrix_t(size_t init_row, size_t init_col,
                           const Type_t &base_data /*= Type_T{}*/)
    : Buffer<Type_t>(init_col * init_row), size_({init_row, init_col}) {
...
//////////////////////////////////////////////////
//////////////////////////////////////////////////
template <class Type_t> bool Matrix_t<Type_t>::IsSquare() const noexcept {
...
bool Matrix_t<Type_t>::operator==(const Matrix_t &that) const {
  ...
  if (that.Empty() || this->Empty() || size_.row != that.size_.row ||
      size_.col != that.size_.col)
    return false;
...
template <class Type_t>
Matrix_t<Type_t>::SetterRaw_t::SetterRaw_t(Type_t *init_data,
                                           size_t init_max) noexcept
    : data_(init_data), size_(init_max) {}
...
Matrix_t<Type_t>::operator[](size_t row) {
  if (data_ == nullptr || row >= size_.row)
    throw std::out_of_range(
        std::string("Tried to get away from massive ROW: ") + __FILE__ +
...
template <class Type_t>
Type_t Matrix_t<Type_t>::SetterRaw_t::operator[](size_t num) const {
  if (num >= size_) {
...
template <class Type_t>
Type_t &Matrix_t<Type_t>::SetterRaw_t::operator[](size_t num) {
  if (num >= size_)
    throw std::out_of_range(std::string("Tried to get away of masive COLUMN!") +
                            __FILE__ + " in line " + std::to_string(__LINE__));
...
template <class Type_t>
Matrix_t<Type_t>::GetterRaw_t::GetterRaw_t(Type_t *init_data,
                                           size_t init_max) noexcept
    : data_(init_data), size_(init_max) {}

template <class Type_t>
Type_t Matrix_t<Type_t>::GetterRaw_t::operator[](size_t num) const {
  if (num >= size_) {
    WARNING("Tried to get away of masive COLUMN!");
    return Type_t{};
  }
...
template <class Type_t>
Matrix_t<Type_t>
Matrix_t<Type_t>::operator+(const Matrix_t<Type_t> &that) const & {
  Matrix_t<Type_t> out_matrix{*this};
  out_matrix += that;
  return out_matrix;
}
template <class Type_t>
Matrix_t<Type_t> Matrix_t<Type_t>::operator+(const Matrix_t<Type_t> &that) && {
  operator+=(that);
  return *this;
}
template <class Type_t>
...
template <class Type_t> Matrix_t<Type_t> &Matrix_t<Type_t>::transpose() & {
  if (!IsSquare()) {
...
//////////////////////////////////////////////////
//////////////////////////////////////////////////
template <class Type_t> void Matrix_t<Type_t>::dump() const {
  if (data_ != nullptr) {
    Type_t *writer = data_;
