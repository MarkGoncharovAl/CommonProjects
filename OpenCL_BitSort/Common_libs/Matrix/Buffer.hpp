...
template <class T> void destroy_arr(T *start, T *end) noexcept {
...
template <class T> void construct_elem(T *place, const T &data) {
  new (place) T{data};
}
...
protected:
  ...
  explicit operator bool() const noexcept {
    return data_ != nullptr && used_ == capacity_;
  }
...
Buffer<T>::Buffer(const Buffer<T> &that)
    : data_((that.capacity_ == 0)
                ? nullptr
                : static_cast<T *>(::operator new(sizeof(T) * that.capacity_))),
      used_(0), capacity_(that.capacity_) {
...
template <class T>
Buffer<T>::Buffer(Buffer<T> &&that) : data_(nullptr), used_(0), capacity_(0) {
  std::swap(data_, that.data_);
  std::swap(used_, that.used_);
  std::swap(capacity_, that.capacity_);
}

template <class T> void Buffer<T>::Swap(Buffer<T> &that) noexcept {
  std::swap(data_, that.data_);
  std::swap(used_, that.used_);
  std::swap(capacity_, that.capacity_);
}

template <class T> void Buffer<T>::DestroyAll() noexcept {
  buf_help_functions::destroy_arr(data_, data_ + used_ * sizeof(T));
  ::operator delete(data_);
}

template <class T> Buffer<T>::~Buffer() noexcept { DestroyAll(); }

template <class T> T *Buffer<T>::Construct(const T &data) {
  if (used_ >= capacity_)
    return nullptr;
...
template <class T> void Buffer<T>::ConstructAll(const T &data) {
...
template <class T> void Buffer<T>::Realloc(size_t cap) {
  T *new_data =
      (cap == 0) ? nullptr : static_cast<T *>(::operator new(sizeof(T) * cap));
