...
enum class Determ_sign { nothing, minus, zero };

template <class T> double Determ(const Matrix_t<T> &matrix);
...
VecRaw_t TriangleMatrix(Matrix_t<double> &matrix,
                        MF::Determ_sign *swappes = nullptr);
...
bool ZeroOutElem(const Matrix_t<double>::SetterRaw_t &base,
                 Matrix_t<double>::SetterRaw_t current_str, size_t num);

enum class CZUMout { nothing, swapped, zero };

CZUMout CreateZeroUnderMatrix(VecRaw_t &vec, size_t num);
...
template <class T> double MF::Determ(const Matrix_t<T> &matrix) {
  if (!matrix.IsSquare()) {
    matrix.dump();
    WARNING("Determinant can't be found on not square matrix! Upper is dump of "
            "this matrix:");
    return 0;
  }
