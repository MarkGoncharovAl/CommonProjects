...
namespace MLib {
[[noreturn]] inline void print_error(std::string info, std::string FILE,
                                     std::size_t LINE) {
...
inline void print_warning(std::string info, std::string FILE,
                          std::size_t LINE) {
  std::cout << Color::Bold_Red << std::endl
            << info << std::endl
...
inline void debug_print(const T &elem1, const T &elem2 = T{}) {
  std::cout << Color::Under_Purple << "Debagging is starting!\n"
            << Color::Reset << elem1 << "\t" << elem2 << Color::Under_Purple
