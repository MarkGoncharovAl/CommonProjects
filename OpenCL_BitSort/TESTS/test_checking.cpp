...
static void CompareData(const std::vector<int> &data,
                        const std::vector<int> &data_check);

int main() {
  constexpr size_t size = 9;
  std::array<std::ifstream, size> files = {
      std::ifstream{"../TESTS/1.txt"}, std::ifstream{"../TESTS/2.txt"},
      std::ifstream{"../TESTS/3.txt"}, std::ifstream{"../TESTS/4.txt"},
      std::ifstream{"../TESTS/5.txt"}, std::ifstream{"../TESTS/6.txt"},
      std::ifstream{"../TESTS/7.txt"}, std::ifstream{"../TESTS/8.txt"},
  ...
  for (std::ifstream &file : files) {
    ...
    std::cout << MLib::Color::Purple
              << "Time passed using std::sort: " << time_found << " mc"
              << std::endl
              << MLib::Color::Reset;
